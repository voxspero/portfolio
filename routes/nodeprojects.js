...
router.get('/', (req, res) => {
	NodeProject.find({}, (err, nodeprojects) => {
		if (err) {
			console.log(err);
		} else {
			res.render('nodeprojects/index', { nodeprojects: nodeprojects });
		}
	}).sort({ year: 'desc' });
});
...
router.get('/:id', (req, res) => {
	NodeProject.findById(req.params.id, function(err, nodeproject) {
		if (err) {
			console.log(err);
		} else {
			res.render('nodeprojects/show', { nodeproject: nodeproject });
		}
	});
});
